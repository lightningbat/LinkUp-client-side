...
MessageContentRight.propTypes = {
    msg_id: PropTypes.string.isRequired,
    msg_type: PropTypes.number.isRequired,
    msg: PropTypes.string.isRequired,
    edited: PropTypes.bool.isRequired
}
export default function MessageContentRight({ msg_id, msg_type, msg, timestamp, edited }) {
    const [showDropdown, setShowDropdown] = useState(false)
    const dropdownBtnRef = useRef(null)

    const device_width = (window.innerWidth > 0) ? window.innerWidth : screen.width;

    const drop_down_menu_direction = device_width > 450 ? {right: '5px'} : {left: '5px'}

    return (
        <div className='message-content-right' id={msg_id}>
